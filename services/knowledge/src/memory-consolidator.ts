...
async function fetchWeeklyConsolidatedMemories(
  ...
  try {
    ...
    const memories = await supabaseQuery(
      ...
      `tenant_id=eq.${tenantId}&created_at=gte.${startTime}&created_at=lte.${endTime}&consolidation_type=in.(topic,user,account)&order=created_at.desc`
    );

    return memories as unknown as ConsolidatedMemory[];
  } catch (error) {
...
async function cleanupOldMemories(env: Env, tenantId: string): Promise<void> {
  ...
  try {
    // v3.6: Delete from Supabase (SINGLE SOURCE OF TRUTH)
    await supabaseMutate(
      env.SUPABASE_URL,
      env.SUPABASE_SERVICE_KEY,
      'consolidated_memories',
      'DELETE',
      undefined,
      `tenant_id=eq.${tenantId}&created_at=lt.${cutoffDate.toISOString()}`
    );
  } catch (error) {
