...
const getDefaultRoute = (viewId: OsViewId) => {
  ...
  const defaultItem = OS_NAV_ITEM_REGISTRY[viewConfig.defaultItemId]
...
const ViewSwitchButton = ({ viewId, isActive }: { viewId: OsViewId; isActive: boolean }) => {
  ...
  const viewConfig = OS_VIEW_CONFIGS[viewId]
  const { allowed } = useViewAccess(viewConfig.defaultItemId)
...
export function OsShell({ children }: { children: React.ReactNode }) {
  ...
  const { scope, setScope } = useWorldScope()
  ...
  useEffect(() => {
    ...
    setInitialized(true)
  }, [initialized, viewFromPath, router])
  ...
  const canWork = featureAccess({ plan: tenant.plan, featureFlags: tenant.featureFlags, featureKey: "worlds.work" }).hasAccess
  const canAccounts = featureAccess({ plan: tenant.plan, featureFlags: tenant.featureFlags, featureKey: "worlds.accounts" }).hasAccess
      ...
      <div className="flex h-screen bg-background text-foreground overflow-hidden">
          ...
          <div className="px-3 py-4 border-b border-border">
            ...
            >
              ...
              <div className="flex-1 text-left">
                ...
                <div className="text-[10px] text-muted-foreground">{tenant.tenantName}</div>
              </div>
            </button>
          </div>
          ...
          <div className="p-2 border-t border-border">
            <button className="w-full flex items-center gap-2 px-2 py-2 rounded-md hover:bg-muted transition-colors">
              ...
              <div className="flex-1 min-w-0 text-left">
                ...
                <p className="text-[10px] text-muted-foreground truncate">Admin • {planLabel(tenant.plan)}</p>
              </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 pb-24">
            {activeNavItem && !activeAccess.allowed ? (
              <div className="max-w-xl border border-border bg-muted/30 text-foreground rounded-xl p-4">
                ...
                <p className="text-xs mt-1">{activeAccess.reason ?? "You do not have access to this view."}</p>
              </div>
            ) : (
              <>
