...
export function CognitiveDrawer({
  ...
  const tabAccess = useMemo(() => {
    const accessFor = (key: string) => featureAccess({ plan: tenant.plan, featureFlags: tenant.featureFlags, featureKey: key }).hasAccess
    return {
      lineage: true,
      spine: accessFor("surfaces.spine"),
      context: accessFor("surfaces.context"),
      knowledge: true,
      think: tenant.plan !== "personal",
      act: tenant.plan !== "personal",
      approve: tenant.plan !== "personal",
      governance: tenant.plan !== "personal",
