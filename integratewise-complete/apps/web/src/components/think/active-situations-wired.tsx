...
export function ActiveSituationsWired({ onEvidenceClick, onActionSelect }: { 
  onEvidenceClick?: (situation: any) => void
  ...
  const countEvidence = (situation: any) => {
    const evidence = situation.evidence || []
    return {
      truth: evidence.filter((e: any) => e.type === "truth").length,
      context: evidence.filter((e: any) => e.type === "context").length,
      ai_chats: evidence.filter((e: any) => e.type === "ai-chat").length,
    }
  }
