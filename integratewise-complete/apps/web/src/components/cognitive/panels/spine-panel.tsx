...
export function SpinePanel({ entityId, entityType = 'account', tenantId }: SpinePanelProps) {
  ...
  const loadEntityData = async () => {
    ...
    try {
      const [entityData, completenessData] = await Promise.all([
        spineClient.getEntity(entityId),
        spineClient.getCompleteness(entityId)
      ])
  ...
  const loadSchemaData = async () => {
    ...
    setIsLoading(true)
    try {
      const schemaData = await spineClient.getSchema({ entity_type: entityType, tenant_id: tenantId })
              ...
              >
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    Seen {field.occurrence_count}x • Last: {new Date(field.last_seen_at).toLocaleDateString()}
                  </div>
                </div>
