...
export function Footer() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (email.trim() && email.includes("@")) {
      // TODO: Wire to your email service (Mailchimp, Resend, etc.)
      setSubmitted(true);
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-2">
              <Input
                ...
                required
                className="max-w-xs bg-white/[0.06] border-white/[0.08] text-white placeholder:text-white/30 focus:border-white/20 rounded-full px-4"
              />
              <Button
                type="submit"
                className="bg-white text-[#0a0a0a] hover:bg-white/90 rounded-full px-5"
              >
                {submitted ? "Sent!" : "Get Started"} <ArrowRight className="ml-1 h-3 w-3" />
              </Button>
            </form>
          </motion.div>
