...
# Services
ALL_SERVICES="web spine-v2 loader normalizer connector cognitive-brain stream-gateway memory-consolidator workflow agents"

# Function to deploy web (Vercel)
deploy_web() {
    local env=$1
    local config="${ENV_SHORT}_web"
    
    echo -e "${BLUE}▶ Deploying web (Vercel) with config: $config${NC}"
    
    cd apps/web
    
    if [ "$env" == "prd" ] || [ "$env" == "production" ]; then
        doppler run --config "$config" -- vercel --prod
    else
        doppler run --config "$config" -- vercel
    fi
