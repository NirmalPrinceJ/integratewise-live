...
app.get('/health', (c) => c.json({ status: 'ok', service: 'Think Engine (Layer 2)', database: 'D1' }));
...
app.get('/v1/signals', async (c) => {
    ...
    try {
        const { results } = await c.env.DB.prepare(`
            SELECT * FROM signals WHERE tenant_id = ? ORDER BY computed_at DESC LIMIT ?
...
app.get('/v1/situations', async (c) => {
    ...
    try {
        const { results } = await c.env.DB.prepare(`
            SELECT * FROM situations WHERE tenant_id = ? ORDER BY created_at DESC LIMIT ?
        `).bind(tenantId, limit).all();
