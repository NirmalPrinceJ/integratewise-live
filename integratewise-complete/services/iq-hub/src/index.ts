...
app.post('/iq/memories/retrieve', async (c) => {
    ...
    try {
        let query = `SELECT * FROM ai_session_memories WHERE confidence >= ? `;
        const params: any[] = [min_confidence];

        if (session_ids && session_ids.length > 0) {
            query += ` AND session_id IN (${session_ids.map(() => '?').join(',')})`;
        ...
        return c.json({ success: true, data: { memories: results } });
    } catch (err: any) {
...
app.get('/conversations', async (c) => {
    ...
    try {
        let query = `SELECT * FROM ai_conversations WHERE tenant_id = ? AND user_id = ? AND status = ?`;
        const params: any[] = [tenantId, userId, status];
...
app.post('/conversations', async (c) => {
    ...
    try {
        await c.env.DB.prepare(`
            INSERT INTO ai_conversations (id, tenant_id, user_id, title, context_type, context_id, model, metadata, expires_at, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).bind(id, tenantId, userId, title || null, context_type || null, context_id || null, model, JSON.stringify(metadata || {}), expiresAt, now, now).run();
...
app.get('/conversations/:id', async (c) => {
    ...
    try {
        ...
        if (results.length === 0) return c.json({ error: 'Conversation not found' }, 404);
...
app.patch('/conversations/:id', async (c) => {
    ...
    try {
        ...
        params.push(conversationId, tenantId, userId);

        await c.env.DB.prepare(`
            UPDATE ai_conversations SET ${updates.join(', ')} WHERE id = ? AND tenant_id = ? AND user_id = ?
        `).bind(...params).run();
