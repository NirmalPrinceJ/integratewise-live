...
export function DocumentsView() {
  ...
  return (
    <div className="p-6 space-y-6 overflow-y-auto h-full">
      ...
      {filtered.length === 0 ? (
        <EmptyState
          variant={searchQuery || typeFilter !== "all" || sourceFilter !== "all" ? "no-results" : "no-documents"}
          ctaLabel={searchQuery || typeFilter !== "all" || sourceFilter !== "all" ? undefined : "Upload Document"}
          onAction={searchQuery || typeFilter !== "all" || sourceFilter !== "all" ? undefined : () => console.log("Upload document")}
        />
      ) : viewMode === "grid" ? (
