...
export function ReadinessBar({ department }: { department: string }) {
  ...
  // Map department name to readiness key with defensive fallback
  const deptKey = department === "ops" ? "bizops" : department;
  const deptReadiness = readiness?.[deptKey];
  
  // Don't render if no readiness data yet
  if (!deptReadiness || !deptReadiness.buckets || deptReadiness.buckets.length === 0) {
    return null;
  }

  const config = stateConfig[deptReadiness.overallState] || stateConfig.off;
