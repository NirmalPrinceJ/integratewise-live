...
// ─── Supabase disconnected — Think Engine uses mock responses ─────────────────
...
function ThinkView({ activeCtx }: { activeCtx: string }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const mockReply = {
        role: "assistant",
        content: `[Offline Mode] The Think Engine requires backend connectivity to process your query: "${input}". When connected, it will analyze your data across all integrated tools and provide contextual intelligence for the ${activeCtx} domain.`
      };
