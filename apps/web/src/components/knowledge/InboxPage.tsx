...
const InboxPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      ...
      try {
        ...
        const sortedData = data.sort((a: Session, b: Session) => new Date(b.ended_at).getTime() - new Date(a.ended_at).getTime());
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      {selectedSession && <SummaryModal session={selectedSession} onClose={() => setSelectedSession(null as Session | null)} />}
    </div>
  );
};
