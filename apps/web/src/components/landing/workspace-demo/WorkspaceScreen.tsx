...
interface DomainView {
  ...
  connectors: { icon: React.ComponentType<{ className?: string }>; label: string }[];
  tableHeader: string;
  tableCount: string;
  columns: string[];
...
const signals = [
  ...
  },
  {
    ...
    label: "Warning",
    severity: "warning" as const,
    summary: "Account status requires attention",
    detail: "Some systems are running near capacity but still operating normally.",
          ...
          <div className="flex items-center gap-1 px-4 py-2 border-b bg-muted/10 overflow-x-auto">
            ...
            {domainViews.map((dv, i) => (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
              >
                {dv.icon && React.createElement(dv.icon as any, { className: "h-3 w-3" })}
                {dv.label}
              </motion.button>
            ))}
            <motion.div
