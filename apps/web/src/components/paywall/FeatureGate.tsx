...
export function FeatureGate({
  ...
  const hasAccess = typeof access === 'object' && access !== null ? access.hasAccess : access
  ...
  const displayName = featureName || (FEATURE_DESCRIPTIONS[featureKey as keyof typeof FEATURE_DESCRIPTIONS] || featureKey)

  return (
    <UpgradePrompt
      featureName={displayName}
      requiredPlan={((typeof access === 'object' ? access.requiredPlan : "starter") || "starter") as EntitlementTier}
      currentPlan={(plan ?? 'free') as EntitlementTier}
      variant={variant}
    />
  )
}
