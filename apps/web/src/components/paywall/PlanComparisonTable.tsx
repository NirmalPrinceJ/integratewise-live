          ...
          return (
            ...
            >
              ...
              <CardHeader className="text-center pb-2">
                ...
                <div className="mt-4">
                  <span className="text-3xl font-bold">{formatPrice(price ?? 0, billingCycle)}</span>
                  {(price ?? 0) !== 0 && (
              ...
              <CardContent className="flex-1 flex flex-col">
                ...
                <div className="space-y-2 mb-4 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatLimit(tier.limits?.users ?? null)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Integrations</span>
                    <span className="font-medium">{formatLimit(tier.limits?.integrations ?? null)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">AI Sessions</span>
                    <span className="font-medium">{formatLimit(tier.limits?.aiSessions ?? null)}</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <div className="mt-auto pt-4">
                  {isCurrent ? (
                    ...
                    <Button variant="outline" className="w-full" onClick={() => onSelectPlan(tier.id as EntitlementTier)}>
                      Contact Sales
                    </Button>
                  ) : (
                    <Button
                      className={cn("w-full", isHighlighted && "bg-primary")}
                      onClick={() => onSelectPlan(tier.id as EntitlementTier)}
                    >
                    ...
                    {PLAN_TIERS.map((tier) => (
                      ...
                      >
                        <div className="flex justify-center">
                          <FeatureValue value={(tier.features as Record<string, boolean | string>)[featureKey] ?? false} />
                        </div>
                      </td>
                    ))}
                  </tr>
