...
export function OnboardingFlowNew({ onComplete, initialUserName, initialEmail }: OnboardingFlowProps) {
  ...
  const [oauthLoading, setOauthLoading] = useState<Set<string>>(new Set());

  // Step 3: File Upload
  const [uploadedFiles, setUploadedFiles] = useState<File[]>([]);
  const [uploadError, setUploadError] = useState("");
  const [fileUploadStatus, setFileUploadStatus] = useState<Record<string, "pending" | "uploading" | "success" | "error">>({});
  const [uploadingCount, setUploadingCount] = useState(0);
  
  // Step 4: Accelerator
  const [selectedAccelerator, setSelectedAccelerator] = useState<string | null>(null);
  
  // Upload file to Store service
  const uploadFileToStore = useCallback(async (file: File) => {
    try {
      const fileKey = file.name;
      setFileUploadStatus(prev => ({ ...prev, [fileKey]: "uploading" }));
      ...
      if (!apiBase) {
        console.warn("[Onboarding] No API base URL configured, skipping file upload");
        setFileUploadStatus(prev => ({ ...prev, [fileKey]: "success" }));
        return null;
      }
      ...
      const result = await fetch(`${apiBase}/api/v1/store/upload`, {
        method: "POST",
        body: formData,
        credentials: "include",
        headers: {
      ...
      if (result.ok) {
        ...
        setFileUploadStatus(prev => ({ ...prev, [fileKey]: "success" }));
        console.log("[Onboarding] File uploaded successfully:", data);
        return data;
      } else {
        setFileUploadStatus(prev => ({ ...prev, [fileKey]: "error" }));
        console.warn("[Onboarding] File upload failed:", result.status);
        return null;
      }
    } catch (err) {
      const fileKey = file.name;
      setFileUploadStatus(prev => ({ ...prev, [fileKey]: "error" }));
      console.warn("[Onboarding] File upload error:", err);
      return null;
    }
  }, [selectedDomain]);
  ...
  const handleFileUpload = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Start non-blocking upload for each file
    validFiles.forEach(file => {
      const fileKey = file.name;
      setFileUploadStatus(prev => ({ ...prev, [fileKey]: "pending" }));
  ...
  // Initiate connector OAuth flow
  const initiateConnectorOAuth = useCallback(async (connectorId: string) => {
    try {
      setOauthLoading(prev => new Set(prev).add(connectorId));

      const apiBase = import.meta.env.VITE_API_BASE_URL || "";
      if (!apiBase) {
        console.warn(`[Onboarding] Connector OAuth for ${connectorId} skipped: No API base URL`);
      ...
      const result = await fetch(`${apiBase}/api/v1/connector/connect/${connectorId}`, {
        method: "POST",
        credentials: "include",
        headers: {
        ...
        body: JSON.stringify({
          callback_url: window.location.origin + "/app?connector_callback=true",
      ...
      if (result.ok) {
        ...
        if (data?.auth_url) {
          // Open OAuth popup
          window.open(data.auth_url, `iw-connect-${connectorId}`, "width=600,height=700,popup=yes");
          // Mark as connected optimistically
          setConnectedConnectors(prev => new Set(prev).add(connectorId));
        }
      } else {
        console.warn(`[Onboarding] Connector OAuth initiation failed for ${connectorId}:`, result.status);
        // Don't block — connector selection is still stored for later setup
      }
    } catch (err) {
      console.warn(`[Onboarding] Connector OAuth for ${connectorId} skipped:`, err);
      // Don't block — connector selection is still stored for later setup
    } finally {
                    ...
                    <div key={key}>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        {category.options.map(option => {
                          ...
                          const isConnected = connectedConnectors.has(option.id);
                          const isLoading = oauthLoading.has(option.id);

                          return (
                            <button
                              key={option.id}
                              onClick={() => {
                                setConnectors(prev => ({ ...prev, [key]: option.id }));
                                initiateConnectorOAuth(option.id);
                              }}
                              disabled={isLoading}
