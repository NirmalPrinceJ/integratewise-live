...
export function LiveSignalsStripWired({ onSignalClick }: { onSignalClick?: (signal: Signal) => void }) {
      ...
      <div className="flex items-center gap-2 overflow-x-auto px-6 py-3 scrollbar-thin scrollbar-thumb-zinc-700">
        {(signals as Signal[]).map((signal) => (
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${severityStyles[signal.severity] || severityStyles.info} hover:bg-opacity-20 transition-colors whitespace-nowrap`}
          >
