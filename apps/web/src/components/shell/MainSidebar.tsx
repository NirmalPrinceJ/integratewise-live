...
import { getRoleConfig, hasPermission, type RoleName } from '@/lib/rbac/roles';
...
export function MainSidebar({
  ...
  const roleConfig = getRoleConfig((role?.id || 'personal-pro') as any);
...
interface NavItemProps {
  item: { id: string; label: string; icon: string; href: string; onClick?: () => void; shortcut?: string; badge?: string };
  pathname: string;
  collapsed: boolean;
}
...
// Generate navigation based on role
function generateNavigation(role: MainSidebarProps['role'], shell: string) {
  const roleConfig = getRoleConfig((role?.id as any) || 'personal-pro');
  const defaultModules = roleConfig?.defaultModules || ['home', 'today', 'tasks'];

  // Base navigation for all roles
  const baseNav: NavItemProps['item'][] = [
