...
type L2ComponentId = "SpineUI" | "ContextUI" | "KnowledgeUI" | "Evidence Drawer" | "Signals" | "Think" | "Act" | "HITL" | "Govern" | "Adjust" | "Repeat" | "Audit Trail" | "Agent Config" | "Digital Twin"

const L2_COMPONENTS: { id: L2ComponentId; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
...
const IMPLEMENTED_TABS: L2ComponentId[] = ["SpineUI", "HITL", "Think", "Act", "Govern", "Digital Twin"]

export function IntelligenceOverlay({ isOpen, onClose, activeCtx }: IntelligenceOverlayProps) {
  const [activeTab, setActiveTab] = useState<L2ComponentId>("SpineUI")
    ...
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 h-[85vh] bg-background border-t shadow-2xl z-[70] flex flex-col rounded-t-2xl overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as L2ComponentId)} className="flex-1 flex flex-col">
...
function SpineView({ activeCtx }: { activeCtx: CTXEnum }) {
  return (
...
function DigitalTwinView({ activeCtx }: { activeCtx: CTXEnum }) {
  const mutations = [
