...
// ── Static fallbacks (used when API unavailable) ──────────────────────────────
...
export function PersonalDashboard() {
  ...
  const { data: projection, loading } = useHydrateProjection<any>("personal");
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-full">
        <Loader2 className="w-8 h-8 animate-spin text-[var(--iw-purple)]" />
      </div>
    );
  }

  // Live data from API → fallback to static mock data
  const todayTasks = projection?.tasks ?? FALLBACK_TASKS;
  const schedule = projection?.schedule ?? FALLBACK_SCHEDULE;

  const streak = projection?.streak ?? 12;
  const completedToday = todayTasks.filter((t: any) => t.done).length;
