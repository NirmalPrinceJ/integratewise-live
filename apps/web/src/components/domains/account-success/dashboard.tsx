...
export function AccountSuccessDashboard() {
  const { data: projection, loading, error } = useHydrateProjection<any>("cs");
  ...
  const accounts = projection?.accounts ?? accountMasterData;
  const tasks = projection?.tasks ?? taskManagerData;
  const engagements = projection?.engagements ?? engagementLogData;
  const risks = projection?.risks ?? riskRegisterData;
  const insights = projection?.insights ?? generatedInsightsData;
