...
export function PipelineView() {
  ...
  const filteredDeals = useMemo(() => {
    return deals.filter(deal => {
      const matchesTab = activeTab === "all" ||
        ...
        (activeTab === "open" && !["closed_won", "closed_lost"].includes(deal.stage as string ?? '')) ||
  ...
  const stats = useMemo(() => {
    const openDeals = deals.filter(d => !["closed_won", "closed_lost"].includes(d.stage as string ?? ''));
...
function DealCard({ deal }: { deal: DealData }) {
  const stage = stageConfig[deal.stage ?? 'discovery'] || { label: deal.stage ?? 'discovery', color: "bg-blue-500" };
  const type = typeConfig[deal.type as keyof typeof typeConfig] || { label: deal.type ?? 'new_business', variant: "outline" as const };
        ...
        <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-start gap-4 flex-1">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-4 text-[11px] font-bold text-muted-foreground/70">
                {deal.account && <><span className="flex items-center gap-1.5"><Building2 className="w-3 h-3" /> {deal.account}</span><span className="opacity-30">·</span></>}
                {deal.owner && <><span className="flex items-center gap-1.5"><User className="w-3 h-3" /> {deal.owner}</span><span className="opacity-30">·</span></>}
                {deal.closeDate && <span className="flex items-center gap-1.5"><Calendar className="w-3 h-3" /> {new Date(deal.closeDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</span>}
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-8 pl-4 md:border-l border-border/40">
            <div className="min-w-[100px] text-right">
              <p className="text-lg font-black tracking-tighter">${((deal.value ?? 0) / 1000).toFixed(0)}k</p>
            ...
            <div className="w-32 hidden xl:block">
              ...
              <div className="h-1.5 bg-secondary/50 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-1000 ${(deal.probability ?? 0) >= 80 ? 'bg-emerald-500' : (deal.probability ?? 0) >= 50 ? 'bg-blue-500' : 'bg-amber-500'}`}
