        ...
        <div className="max-w-6xl mx-auto px-6 py-4">
          ...
          <div className={`p-4 rounded-xl ${status.bg} flex items-center justify-between`}>
            <div className="flex items-center gap-3">
              <StatusIcon className={`w-6 h-6 ${status.color} ${(status as any).animate ? 'animate-spin' : ''}`} />
              <div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              ...
              {showLogs && (
                <div className="border-t border-gray-200 bg-gray-900 p-4 font-mono text-sm overflow-x-auto max-h-[400px] overflow-y-auto">
                  {logs.map((log, idx) => {
                    const time = new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit' as const, minute: '2-digit' as const, second: '2-digit' as const, fractionalSecondDigits: 3 } as any)
