...
const fetcher = (url: string) => apiFetch<any>(url, {}, "GovernDashboard").then(r => r?.data ?? r)
...
export function GovernDashboard({ policies: initialPolicies, approvalQueue: initialQueue }: GovernDashboardProps) {
    ...
    const { openEvidence = () => {} } = (useEvidence?.() || {}) as any
    ...
    const handleApprove = async (itemId: string) => {
        ...
        try {
            await apiFetch("/api/v1/cognitive/govern/approve", {
                method: "POST",
                body: { itemId, decision: "approved" },
            }, "GovernApprove")
            refreshQueue()
        } catch (error) {
            console.error("[GovernDashboard] Failed to approve:", error)
        } finally {
    ...
    const handleReject = async (itemId: string) => {
        setProcessingId(itemId)
        try {
            await apiFetch("/api/v1/cognitive/govern/reject", {
                method: "POST",
                body: { itemId, decision: "rejected" },
            }, "GovernReject")
            refreshQueue()
        } catch (error) {
            console.error("[GovernDashboard] Failed to reject:", error)
        } finally {
            setProcessingId(null)
        }
    }
