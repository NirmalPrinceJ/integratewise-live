        ...
        <div className="flex gap-1 flex-wrap">
          {tabs.map(t => (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-2 text-xs font-semibold rounded-lg transition-all ${activeTab === t.id ? "bg-sky-500 text-white" : "bg-secondary text-muted-foreground hover:text-foreground"}`}
            >
          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <Button size="sm" className="bg-sky-500 hover:bg-sky-600 text-xs h-8 gap-1"><UserPlus className="w-3 h-3" />Invite</Button>
              </div>
                  ...
                  return (
                    <Card key={member.id} className="border shadow-sm">
                      <CardContent className="p-3 flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-sky-500 flex items-center justify-center text-white text-xs font-bold shrink-0">{member.initials}</div>
        ...
        {activeTab === "team-sync" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button size="sm" className="bg-sky-500 hover:bg-sky-600 text-xs h-8 gap-1"><Plus className="w-3 h-3" />New Rule</Button>
            </div>
            {SYNC_RULES.map(rule => (
              ...
              <CardContent className="p-4">
                ...
                <div className="flex gap-3">
                  {[
                    ...
                    <button key={theme.label} className={`flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all ${theme.active ? "border-sky-500" : "border-transparent hover:border-border"}`}>
              ...
              <CardContent className="p-4">
                ...
                <div className="flex gap-3">
                  {["Compact", "Comfortable", "Spacious"].map((d, i) => (
                    <button key={d} className={`flex-1 py-2 text-xs font-medium rounded-lg border-2 transition-all ${i === 1 ? "border-sky-500 bg-sky-500/5" : "border-border hover:border-sky-500/30"}`}>{d}</button>
                  ))}
                </div>
              </CardContent>
            </Card>
