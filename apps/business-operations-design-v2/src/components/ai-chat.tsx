      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-sky-500/10 to-teal-500/10 flex items-center justify-center mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-lg w-full">
              {SUGGESTED_PROMPTS.map((prompt, i) => (
                ...
                >
                  <prompt.icon className="w-4 h-4 text-muted-foreground group-hover:text-sky-500 shrink-0" />
                  <div>
        ...
        {messages.map(msg => (
          <motion.div key={msg.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : ""}`}>
            {msg.role === "assistant" && (
              <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-sky-500 to-teal-500 flex items-center justify-center shrink-0 mt-0.5">
                <Bot className="w-3.5 h-3.5 text-white" />
              </div>
            )}
            <div className={`max-w-[85%] ${msg.role === "user" ? "bg-sky-500 text-white rounded-2xl rounded-br-md px-4 py-2.5" : ""}`}>
              {msg.role === "assistant" && (
                  ...
                  {msg.suggestions && (
                    <div className="flex flex-wrap gap-1.5">
                      {msg.suggestions.map((s, i) => (
                        <button key={i} onClick={() => sendMessage(s)} className="text-[10px] font-medium px-2.5 py-1 rounded-full bg-secondary hover:bg-sky-500/10 hover:text-sky-500 transition-all border border-border">
                          {s}
                        </button>
                      ))}
                    </div>
            ...
            {msg.role === "user" && (
              <div className="w-7 h-7 rounded-lg bg-[#1E293B] flex items-center justify-center shrink-0 mt-0.5">
        ...
        {isTyping && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-3">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-sky-500 to-teal-500 flex items-center justify-center shrink-0">
              <Bot className="w-3.5 h-3.5 text-white" />
            </div>
      ...
      <div className="px-6 py-3 border-t border-border shrink-0">
        <form onSubmit={e => { e.preventDefault(); sendMessage(input); }} className="flex items-center gap-2">
          <input
            ...
            className="flex-1 bg-secondary/50 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-sky-500/20 border border-border"
          />
          <Button type="submit" size="icon" disabled={!input.trim()} className="bg-sky-500 hover:bg-sky-600 rounded-xl w-10 h-10 shrink-0">
