...
import { Check, Zap, Crown, Building2, ArrowRight, CreditCard, Clock, BarChart3, Users, Plug, Eye } from "lucide-react";
import { Card, CardContent } from "./ui/card";
import { Button } from "./ui/button";
import { Badge } from "./ui/badge";
import { ScrollArea } from "./ui/scroll-area";
import { Progress } from "./ui/progress";
import FigmaPricingPage from "../imports/IntegrateWiseBusinessOperationsDesign-251-4301";
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-6">
        ...
        {/* Toggle: Detailed Pricing View */}
        <div className="flex items-center gap-2">
          <Button
            variant={showDetailedPricing ? "default" : "outline"}
            size="sm"
            className={`text-xs gap-1.5 ${showDetailedPricing ? "bg-sky-500 hover:bg-sky-600" : ""}`}
            onClick={() => setShowDetailedPricing(!showDetailedPricing)}
          >
            <Eye className="w-3.5 h-3.5" />
            {showDetailedPricing ? "Back to Overview" : "View Detailed Pricing"}
          </Button>
        </div>

        {showDetailedPricing ? (
          /* Figma Pricing Design */
          <div className="relative w-full overflow-hidden rounded-xl border border-border shadow-sm bg-white">
            <div className="overflow-auto max-h-[calc(100vh-200px)]">
              <div className="min-w-[900px] relative" style={{ height: "4000px" }}>
                <FigmaPricingPage />
              </div>
            </div>
          </div>
                ...
                <Card key={plan.id} className={`border shadow-sm relative ${plan.popular ? "ring-2 ring-[#F54476]" : ""} ${isCurrent ? "bg-secondary/30" : ""}`}>
                  ...
                  <CardContent className="p-5">
                    ...
                    {isCurrent ? (
                      ...
                      <Button className={`w-full text-xs ${plan.popular ? "bg-sky-500 hover:bg-sky-600" : "bg-secondary hover:bg-accent"}`}>
