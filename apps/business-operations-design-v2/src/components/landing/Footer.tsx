...
import { Github, Twitter, Linkedin, Mail, ArrowUpRight, ArrowRight, Check } from "lucide-react";
...
export function Footer() {
  ...
  return (
    <footer className="bg-[#1E2A4A] pt-16 md:pt-20 pb-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-10 lg:gap-8 mb-16">
            ...
            <div className="flex gap-3">
              {[
                ...
                <a
                  ...
                  className="w-9 h-9 rounded-lg bg-white/5 flex items-center justify-center text-white/30 hover:bg-white/10 hover:text-white transition-all"
                >
          ...
          {footerLinks.map((column, i) => (
            <div key={i}>
              <h4 className="text-[11px] font-black text-white/60 uppercase tracking-[0.2em] mb-5">
                {column.title}
              </h4>
                ...
                {column.links.map((link, j) => (
                  <li key={j}>
                    <button
                      ...
                      className="text-white/40 hover:text-[#F54476] font-medium text-[13px] transition-colors flex items-center gap-1 group/link"
                    >
                      {link.label}
              ...
              {subscribed ? (
                ...
                <form onSubmit={handleSubscribe} className="flex gap-2">
                  <input
                    ...
                    required
                    className="flex-1 md:w-56 px-4 py-2.5 bg-white/10 text-white placeholder:text-white/25 border border-white/10 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#F54476]/40 focus:border-[#F54476]/40"
                  />
                  <button
                    type="submit"
                    className="px-5 py-2.5 bg-[#F54476] hover:bg-[#E03A66] text-white rounded-lg font-bold text-sm transition-colors shrink-0 flex items-center gap-1.5"
                  >
        ...
        <div className="pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-1 text-[11px] font-bold text-white/20 uppercase tracking-widest">
            <p>© {currentYear} IntegrateWise Inc.</p>
            <p className="hidden sm:block">San Francisco · Bangalore</p>
          </div>
          <div className="flex flex-wrap gap-x-5 gap-y-1">
            {[
              ...
              <button
                key={i}
                onClick={() => { window.location.hash = link.hash; window.scrollTo(0, 0); }}
                className="text-[11px] font-bold text-white/20 hover:text-[#F54476] transition-colors uppercase tracking-widest"
              >
                {link.label}
              </button>
            ))}
