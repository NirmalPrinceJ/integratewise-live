...
export function HeroSection({
  ...
  const bg = dark ? "bg-[#1E2A4A] text-white" : "bg-white text-[#1E2A4A]";
      ...
      <section className={`pt-28 md:pt-36 pb-16 md:pb-24 ${bg}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <FadeIn>
              {badge && (
                <span className={`inline-block py-1 px-4 rounded-full text-xs font-bold uppercase tracking-wider mb-5 ${dark ? "bg-white/10 text-white/70" : "bg-[#3F5185]/10 text-[#3F5185]"}`}>
                  {badge}
                </span>
              )}
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold leading-tight mb-6">
                {title}{" "}
                {highlight && <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#3F5185] to-[#F54476]">{highlight}</span>}
              </h1>
              <p className={`text-lg md:text-xl ${descColor} leading-relaxed mb-8`}>{description}</p>
              <div className="flex flex-col sm:flex-row gap-3">
                {primaryCta && (
                  <button onClick={() => { window.location.hash = primaryCta.hash; }} className="px-7 py-3.5 bg-[#F54476] hover:bg-[#E03A66] text-white rounded-full font-bold text-base shadow-lg shadow-[#F54476]/20 transition-all flex items-center gap-2 group">
                    {primaryCta.label} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                  </button>
                )}
                {secondaryCta && (
                  <button onClick={() => { window.location.hash = secondaryCta.hash; }} className={`px-7 py-3.5 border-2 rounded-full font-bold text-base transition-all ${dark ? "border-white/20 text-white hover:bg-white/10" : "border-slate-200 text-slate-700 hover:border-[#3F5185] hover:text-[#3F5185]"}`}>
                    {secondaryCta.label}
                  </button>
                )}
              </div>
...
export function FeatureGrid({ badge, title, subtitle, features, columns = 3, accentColor = "#3F5185" }: FeatureGridProps) {
    ...
    <section className="py-16 md:py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <FadeIn className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
          {badge && <span className="inline-block py-1 px-4 rounded-full bg-[#3F5185]/10 text-[#3F5185] text-xs font-bold uppercase tracking-wider mb-4">{badge}</span>}
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-[#1E2A4A] mb-4">{title}</h2>
          ...
          {features.map((f, i) => (
            <FadeIn key={i} delay={i * 0.07}>
              <div className="bg-white border border-slate-200 rounded-2xl p-6 md:p-8 hover:shadow-lg hover:border-slate-300 transition-all group h-full">
                ...
                <h3 className="text-lg font-bold text-[#1E2A4A] mb-2">{f.title}</h3>
...
export function FeatureWithImage({ badge, title, description, bulletPoints, image, imageAlt, imageRight = true, cta, accentColor = "#3F5185" }: FeatureWithImageProps) {
  return (
    <section className="py-16 md:py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center ${imageRight ? "" : "lg:[direction:rtl] lg:[&>*]:![direction:ltr]"}`}>
          <FadeIn>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-[#1E2A4A] mb-4 leading-tight">{title}</h2>
...
export function StatsBar({ stats, dark = false, title, subtitle }: StatsBarProps) {
  const bg = dark ? "bg-[#1E2A4A] text-white" : "bg-slate-50 text-[#1E2A4A]";
          ...
          {stats.map((s, i) => (
            <FadeIn key={i} delay={i * 0.08} className="text-center">
              <div className="text-3xl sm:text-4xl md:text-5xl font-black mb-1">
                <span className={dark ? "text-[#F54476]" : "text-[#3F5185]"}>{s.value}</span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {title && (
          <FadeIn className="text-center mb-12">
            <h2 className="text-2xl md:text-3xl font-extrabold text-[#1E2A4A]">{title}</h2>
          </FadeIn>
        )}
          ...
          {testimonials.map((t, i) => (
            <FadeIn key={i} delay={i * 0.1}>
              <div className="bg-white rounded-2xl p-6 md:p-8 shadow-sm border border-slate-100 h-full flex flex-col">
                ...
                <Quote className="w-8 h-8 text-[#3F5185]/20 mb-3" />
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
                  {t.avatar ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-[#3F5185] flex items-center justify-center text-white font-bold text-sm">
                      {t.author.split(" ").map(n => n[0]).join("")}
                    </div>
                  )}
                  <div>
                    <div className="text-sm font-bold text-[#1E2A4A]">{t.author}</div>
...
export function CTASection({ title, description, primaryCta, secondaryCta, dark = true }: CTAProps) {
  return (
    <section className={`py-16 md:py-24 ${dark ? "bg-[#1E2A4A]" : "bg-white"}`}>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <FadeIn>
          <h2 className={`text-2xl sm:text-3xl md:text-4xl font-extrabold mb-4 ${dark ? "text-white" : "text-[#1E2A4A]"}`}>
            {title}
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            ...
            {secondaryCta && (
              <button onClick={() => { window.location.hash = secondaryCta.hash; }} className={`px-7 py-3.5 border-2 rounded-full font-bold text-base transition-all ${dark ? "border-white/20 text-white hover:bg-white/10" : "border-slate-200 text-slate-700 hover:border-[#3F5185]"}`}>
                {secondaryCta.label}
              </button>
            )}
          </div>
  ...
  return (
    <section className="py-16 md:py-24 bg-white">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <FadeIn className="text-center mb-12">
          <h2 className="text-2xl md:text-3xl font-extrabold text-[#1E2A4A] mb-3">{title}</h2>
          ...
          {faqs.map((faq, i) => (
            <FadeIn key={i} delay={i * 0.05}>
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                ...
                >
                  <span className="text-sm md:text-base font-semibold text-[#1E2A4A] pr-4">{faq.question}</span>
...
export function TimelineSection({ title, subtitle, steps, accentColor = "#3F5185" }: TimelineProps) {
  return (
    <section className="py-16 md:py-24 bg-slate-50">
              ...
              <FadeIn key={i} delay={i * 0.1}>
                <div className="flex gap-5 md:gap-8 items-start">
                  ...
                  <div className="pt-2 flex-1">
                    <h3 className="text-lg font-bold text-[#1E2A4A] mb-1">{step.title}</h3>
          ...
          {members.map((m, i) => (
            <FadeIn key={i} delay={i * 0.08}>
              <div className="text-center group">
                <div className="w-24 h-24 mx-auto rounded-full overflow-hidden bg-[#3F5185]/10 mb-4 group-hover:scale-105 transition-transform">
                  {m.image ? (
                    <ImageWithFallback src={m.image} alt={m.name} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-[#3F5185] font-bold text-2xl">
                      {m.name.split(" ").map(n => n[0]).join("")}
                    </div>
                  )}
                </div>
                <h3 className="text-base font-bold text-[#1E2A4A]">{m.name}</h3>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden hover:shadow-lg transition-all cursor-pointer group h-full flex flex-col">
                ...
                <div className="p-5 flex-1 flex flex-col">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-[#3F5185] bg-[#3F5185]/10 px-2 py-0.5 rounded-full">{post.category}</span>
                    <span className="text-[10px] text-slate-400">{post.date}</span>
                    <span className="text-[10px] text-slate-400">· {post.readTime}</span>
                  </div>
                  <h3 className="text-base font-bold text-[#1E2A4A] mb-2 group-hover:text-[#3F5185] transition-colors">{post.title}</h3>
                  <p className="text-sm text-slate-500 flex-1">{post.excerpt}</p>
                  <div className="flex items-center gap-1.5 mt-4 text-sm font-bold text-[#3F5185] group-hover:gap-2.5 transition-all">
    ...
    <section className="py-16 md:py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
          <FadeIn>
            <h2 className="text-2xl md:text-3xl font-extrabold text-[#1E2A4A] mb-4">{title}</h2>
            ...
            <div className="space-y-5">
              {[
                ...
                <div key={i} className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-xl bg-[#3F5185]/10 flex items-center justify-center shrink-0">
                    <item.icon className="w-5 h-5 text-[#3F5185]" />
                  </div>
                  <div>
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-0.5">{item.label}</div>
                    {item.href ? (
                      <a href={item.href} className="text-sm font-medium text-[#1E2A4A] hover:text-[#3F5185] transition-colors">{item.value}</a>
                    ) : (
                      <span className="text-sm font-medium text-[#1E2A4A]">{item.value}</span>
                    )}
                  </div>
                </div>
              ))}
              ...
              <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="bg-slate-50 rounded-2xl p-6 md:p-8 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" required className="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3F5185]/20 focus:border-[#3F5185]" />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <input type="email" required className="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3F5185]/20 focus:border-[#3F5185]" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-600 mb-1.5">Company</label>
                  <input type="text" className="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3F5185]/20 focus:border-[#3F5185]" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-600 mb-1.5">How can we help?</label>
                  <select className="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#3F5185]/20 focus:border-[#3F5185]">
                ...
                <div>
                  ...
                  <textarea rows={4} className="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-lg text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#3F5185]/20 focus:border-[#3F5185]" />
                </div>
                <button type="submit" className="w-full py-3 bg-[#3F5185] hover:bg-[#344573] text-white rounded-lg font-bold text-sm transition-colors">
                  Send Message
                </button>
              </form>
            )}
        ...
        <FadeIn>
          <div className="border border-slate-200 rounded-2xl overflow-hidden">
            <div className="grid grid-cols-3 bg-slate-50 border-b border-slate-200">
              ...
              <div className="p-4 text-sm font-bold text-center text-[#3F5185]">{usLabel}</div>
              ...
              <div key={i} className={`grid grid-cols-3 ${i < rows.length - 1 ? "border-b border-slate-100" : ""}`}>
                ...
                <div className="p-4 flex justify-center">
                  {typeof row.us === "boolean" ? (
                    ...
                    <span className="text-sm font-medium text-[#3F5185]">{row.us}</span>
                  )}
                </div>
                <div className="p-4 flex justify-center">
...
export function NewsletterSection({ title = "Stay Updated", description }: NewsletterProps) {
  ...
  return (
    <section className="py-16 md:py-20 bg-[#3F5185]">
        ...
        <FadeIn>
          ...
          {done ? (
            ...
            <form onSubmit={(e) => { e.preventDefault(); if (email.trim()) setDone(true); }} className="flex gap-3 max-w-md mx-auto">
              ...
              <button type="submit" className="px-6 py-3 bg-[#F54476] hover:bg-[#E03A66] text-white rounded-lg font-bold text-sm transition-colors shrink-0">Subscribe</button>
            </form>
          )}
        </FadeIn>
      </div>
    ...
    <section className="py-16 md:py-24 bg-slate-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`grid grid-cols-1 ${gridCols} gap-6 max-w-5xl mx-auto`}>
          {tiers.map((tier, i) => (
            <FadeIn key={i} delay={i * 0.1}>
              <div className={`rounded-2xl p-6 md:p-8 h-full flex flex-col ${tier.highlighted ? "bg-[#1E2A4A] text-white ring-2 ring-[#F54476] shadow-xl" : "bg-white border border-slate-200 shadow-sm"}`}>
                <h3 className={`text-lg font-bold mb-1 ${tier.highlighted ? "text-white" : "text-[#1E2A4A]"}`}>{tier.name}</h3>
                ...
                <ul className="space-y-2.5 flex-1 mb-6">
                  {tier.features.map((f, j) => (
                    <li key={j} className="flex items-start gap-2.5 text-sm">
                      <Check className={`w-4 h-4 mt-0.5 shrink-0 ${tier.highlighted ? "text-[#F54476]" : "text-emerald-500"}`} />
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-lg font-bold text-sm transition-colors ${tier.highlighted ? "bg-[#F54476] hover:bg-[#E03A66] text-white" : "bg-[#3F5185] hover:bg-[#344573] text-white"}`}
                >
                  {tier.cta}
                </button>
              </div>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8">
          {tabs.map((tab, i) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all ${activeTab === i ? "bg-[#3F5185] text-white shadow-lg" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
            >
