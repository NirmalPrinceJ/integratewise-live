    ...
    <section className="pt-28 md:pt-36 pb-16 md:pb-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <FadeIn className="text-center max-w-3xl mx-auto mb-12">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-[#1E2A4A] leading-tight mb-5">
            ...
            return (
              <FadeIn key={tier.sku} delay={i * 0.08}>
                <div className={`rounded-2xl flex flex-col h-full transition-all relative ${
                  tier.highlighted
                    ? "bg-[#1E2A4A] text-white ring-2 ring-[#F54476] shadow-2xl"
                    : "bg-white border border-slate-200 hover:shadow-lg"
                }`}>
                  {tier.badge && (
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-1 bg-[#F54476] rounded-full text-white text-[10px] font-black uppercase tracking-wider shadow-lg shadow-[#F54476]/30 whitespace-nowrap">
                      {tier.badge}
                    </div>
                  )}
                  ...
                  <div className="p-6 flex flex-col h-full">
                    <div className="mb-5">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className={`text-lg font-bold ${tier.highlighted ? "text-white" : "text-[#1E2A4A]"}`}>
                          {tier.name}
                        </h3>
                        <span className={`text-[9px] font-mono tracking-wider px-2 py-0.5 rounded ${
                          tier.highlighted ? "bg-white/10 text-white/40" : "bg-slate-100 text-slate-300"
                        }`}>
                          {tier.sku}
                        </span>
                      </div>
                      ...
                      <div className="flex items-baseline gap-1">
                        ...
                        {tier.priceMonthly > 0 && (
                          <span className={`text-sm ${tier.highlighted ? "text-white/35" : "text-slate-400"}`}>
                            /{tier.billingLabel}
                          </span>
                        )}
                      </div>
                      {annual && tier.annualSavings && (
                        <p className="text-[11px] font-bold text-emerald-400 mt-1">{tier.annualSavings}</p>
                      )}
                    </div>

                    <p className={`text-[13px] leading-relaxed mb-5 ${tier.highlighted ? "text-white/50" : "text-slate-500"}`}>
                      {tier.description}
                    </p>
                    ...
                    <ul className="space-y-2 flex-1 mb-6">
                      {tier.features.map((feature, j) => (
                        <li key={j} className="flex items-start gap-2 text-[13px]">
                          ...
                          <span className={tier.highlighted ? "text-white/70" : "text-slate-600"}>
                            {feature}
                          </span>
                        </li>
                      ))}
                    ...
                    <button
                      ...
                      className={`w-full py-3 rounded-xl font-bold text-sm transition-all ${
                        tier.highlighted
                          ? "bg-[#F54476] hover:bg-[#E03A66] text-white shadow-lg shadow-[#F54476]/25"
...
function FeatureComparisonSection() {
  ...
  const renderValue = (val: boolean | string) => {
    ...
    if (val === false) return <X className="w-4 h-4 text-slate-200 mx-auto" />;
    return <span className="text-sm font-medium text-[#1E2A4A]">{val}</span>;
  };
        ...
        <FadeIn>
          <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            ...
            <div className="grid grid-cols-5 border-b border-slate-200 bg-slate-50">
              ...
              {tierNames.map((name, i) => (
                <div key={i} className={`p-4 text-sm font-bold text-center ${
                  name === "Professional" ? "text-[#F54476]" : "text-[#1E2A4A]"
                }`}>
                  {name}
                </div>
              ))}
            ...
            {categories.map((category) => (
              <div key={category}>
                {/* Category header */}
                <div className="grid grid-cols-5 bg-[#3F5185]/5 border-b border-slate-100">
                  <div className="col-span-5 px-4 py-2.5 text-[11px] font-black text-[#3F5185] uppercase tracking-widest">
                    {category}
                  </div>
                </div>
...
const CATEGORY_META: Record<string, { icon: any; label: string; color: string }> = {
  capacity: { icon: Zap, label: "Capacity", color: "#3F5185" },
  security: { icon: Shield, label: "Security & Compliance", color: "#3F5185" },
  support: { icon: Headphones, label: "Support", color: "#3F5185" },
  integration: { icon: Puzzle, label: "Integration", color: "#3F5185" },
};
...
function AddOnsSection() {
  ...
  return (
    <section className="py-16 md:py-24 bg-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <FadeIn className="text-center mb-12">
          <span className="inline-block py-1 px-4 rounded-full bg-[#3F5185]/10 text-[#3F5185] text-xs font-bold uppercase tracking-wider mb-4">
            Add-Ons
          </span>
          <h2 className="text-2xl md:text-3xl font-extrabold text-[#1E2A4A] mb-3">
            Customize Your Plan
                  ...
                  {addOns.map((addon) => (
                    <div key={addon.sku} className="bg-white border border-slate-200 rounded-xl p-5 hover:shadow-md hover:border-slate-300 transition-all flex items-start gap-4">
                      <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center shrink-0">
                        <Plus className="w-5 h-5 text-[#3F5185]" />
                      </div>
...
function EnterpriseCTA() {
  return (
    <section className="py-16 md:py-24 bg-[#1E2A4A]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <FadeIn>
          <span className="inline-block py-1 px-4 rounded-full bg-white/10 text-white/60 text-xs font-bold uppercase tracking-wider mb-5">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <button
              ...
              className="px-7 py-3.5 bg-[#F54476] hover:bg-[#E03A66] text-white rounded-full font-bold text-base shadow-lg shadow-[#F54476]/20 transition-all flex items-center gap-2 group"
            >
