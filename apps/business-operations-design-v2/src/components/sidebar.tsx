...
const DEEP_DIVE_DOMAINS = [
  { id: "account-success", label: "Account Success", icon: "💚", color: "text-emerald-400" },
  { id: "personal", label: "Personal", icon: "👤", color: "text-violet-400" },
  { id: "revops", label: "RevOps", icon: "📈", color: "text-sky-400" },
  { id: "salesops", label: "SalesOps", icon: "🎯", color: "text-teal-400" },
];
  ...
  return (
    <div className={`h-full flex flex-col border-r border-[#1E293B] bg-[#0C1222] transition-all duration-300 ${collapsed ? "w-[56px]" : "w-[220px]"}`}>
      ...
      <div className="flex-1 overflow-y-auto px-2 scrollbar-none">
        ...
        {/* Deep Dive Domains */}
        <SectionHeader label="Deep Dive" expanded={deepDiveExpanded} onToggle={() => setDeepDiveExpanded(!deepDiveExpanded)} />
        {(deepDiveExpanded || collapsed) && onDeepDive && (
          <nav className="space-y-0.5">
            {DEEP_DIVE_DOMAINS.map((domain) => (
              <button
                key={domain.id}
                onClick={() => onDeepDive(domain.id)}
                className={`w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg text-[13px] transition-all text-slate-400 hover:bg-white/5 hover:text-slate-200 ${collapsed ? "justify-center px-0" : ""}`}
                title={collapsed ? domain.label : undefined}
              >
                <Compass className={`w-3.5 h-3.5 shrink-0 ${domain.color}`} />
                {!collapsed && (
