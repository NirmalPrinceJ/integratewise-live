...
function PersonalView({ summary }: { summary: any }) {
  const stats = [
    { label: "My Tasks", value: "12", sub: "4 due today", color: "text-sky-500", icon: Activity },
          ...
          <Card key={i} className="border-none shadow-sm hover:shadow-md transition-all cursor-pointer group">
            <CardContent className="p-4">
              <div className="flex items-start justify-between">
                ...
                <div className="p-2 rounded-lg bg-secondary group-hover:bg-sky-500/10 transition-colors">
                  <stat.icon className="w-4 h-4 text-muted-foreground group-hover:text-sky-500" />
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <Card key={kpi.id} className="border-none shadow-sm">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="p-1.5 rounded-lg bg-sky-500/10"><Target className="w-3.5 h-3.5 text-sky-500" /></div>
                ...
                {kpi.sparkline && (
                  <div className="flex items-end gap-px h-3 mt-2">
                    {kpi.sparkline.map((v: number, j: number) => {
                      ...
                      return <div key={j} className={`flex-1 rounded-sm ${j === kpi.sparkline!.length - 1 ? "bg-sky-500" : "bg-sky-500/20"}`} style={{ height: `${height}px` }} />;
                    })}
                  </div>
                )}
              </CardContent>
