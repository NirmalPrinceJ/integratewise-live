    ...
    <section className="py-24 md:py-32 bg-gray-50 border-t border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16 md:mb-24">
          <h2 className="text-3xl md:text-4xl font-black text-gray-900 mb-6 uppercase tracking-tight font-['Plus_Jakarta_Sans']">The 16 Pillars of Integration Intelligence</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
           {diffs.map((diff, i) => (
             <motion.div
               ...
               className="p-8 bg-white rounded-[32px] border border-gray-200 shadow-sm hover:shadow-2xl hover:border-emerald-200 transition-all group"
             >
                <div className="flex items-center gap-4 mb-6">
                   <div className="w-10 h-10 rounded-xl bg-emerald-50 text-[#059669] flex items-center justify-center font-black text-sm group-hover:bg-[#059669] group-hover:text-white transition-all shadow-inner font-['JetBrains_Mono']">
                      {String(i + 1).padStart(2, '0')}
                   </div>
                   <h3 className="font-black text-gray-900 text-sm uppercase tracking-tight leading-tight font-['Plus_Jakarta_Sans']">{diff.title}</h3>
                </div>
                <p className="text-sm text-gray-500 leading-relaxed font-medium">{diff.desc}</p>
             </motion.div>
           ))}
        </div>
      </div>
